...
export default function SentraHUD() {
  ...
  useEffect(() => {
    ...
    const seq = async () => {
      ...
      addLog(`${diag.microphone ? '🟢' : '🟡'} MIC: ${diag.microphone ? 'OK' : 'Conectando...'}`, diag.microphone ? 'ok' : 'warn');
      addLog(`${diag.geolocation ? '🟢' : '🟡'} GEO: ${diag.geolocation ? 'OK' : 'Conectando...'}`, diag.geolocation ? 'ok' : 'warn');
