...
// Elimina 1 request a Supabase por alerta biométrica (antes: ~1 req cada 1s
// cuando la coherencia estaba baja o el HR salía de rango).
...
(async () => {
  ...
  } catch { /* offline o supabase indisponible */ }
})();
...
class SensorPipeline {
  ...
  private processReading(reading: SensorReading) {
    ...
    // ── Canal de telemetría minimalista (auditoría 2026-01) ────────────────
    // Regla: SOLO envía si bpm >= 110 o alert_type === 'CRITICAL'.
    // Modo silencioso automático desde el filtro. Fire-and-forget, sin retry.
    // Misma lógica de "critical" que usa persistAlert() para consistencia.
    const isCritical = alerts.includes('HIGH_HR') || alerts.includes('STRESS_DETECTED');
    void sendTelemetry({
      bpm:        reading.data.heartRate,
      alert_type: isCritical ? 'CRITICAL' : 'NORMAL',
    });
  }
  ...
  private persistAlert(_reading: SensorReading, alerts: string[]) {
    (async () => {
      try {
        // Diff #6: usar el user id cacheado. Sin fetch a supabase.auth.getUser().
